...
${validFiles > 0 ? "데이터가 정상입니다. [데이터 복구] 버튼을 눌러 진행하세요." : "⚠️ 유효한 데이터 구조를 찾지 못했습니다."}
    ...
    const handleTestConnection = async () => {
        ...
        if (!apiKey || apiKey.trim() === '') {
            announceStatus('API Key를 입력해주세요.');
            return;
        }
        
        // [FIX] Validate prefix logic
        if (!apiKey.trim().startsWith('AIza')) {
            announceStatus("API Key는 'AIza'로 시작해야 합니다. 올바른 키인지 확인해주세요.");
            return;
        }
        ...
        if (success) {
            announceStatus('연결 성공: 유효한 API Key입니다.');
        } else {
    ...
    const handleSaveConfig = async () => {
        ...
        if (trimmedKey && !trimmedKey.startsWith('AIza')) {
            const isConfirmed = await requestConfirm("경고: API Key 형식이 올바르지 않아 보입니다. (AIza로 시작해야 함)\n그래도 저장하시겠습니까?", {
                title: 'API Key 형식 경고',
                confirmLabel: '저장',
                variant: 'warning'
            });
    ...
    const handleClearApiKey = async () => {
        const isConfirmed = await requestConfirm('저장된 API Key를 현재 브라우저 세션에서 제거하시겠습니까?', {
            title: 'API Key 제거',
            confirmLabel: '제거',
            variant: 'danger'
        });
        ...
        announceStatus('API Key가 제거되었습니다.');
    };
                    ...
                    {activeTab === 'SYSTEM' && (
                        <div className="space-y-6 animate-fade-in">
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 mb-1">연계율 목표값 (Linkage Target %)</label>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button 
                                            ...
                                            className="px-4 rounded-xl font-bold text-xs flex items-center gap-2 border bg-white border-slate-300 text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <Eraser size={16}/>
                                            키 삭제
                                        </button>
