...
export const SettingsModal: React.FC<SettingsModalProps> = ({ 
    ...
    const handleSignatureFileChange = (role: 'safety' | 'site') => (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onerror = () => {
            if (mountedRef.current) alert('서명 이미지 읽기에 실패했습니다.');
        };
    ...
    const handleVerifyClick = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files.length > MAX_BACKUP_FILE_COUNT) {
            alert(`파일은 최대 ${MAX_BACKUP_FILE_COUNT}개까지 검사할 수 있습니다.`);
            e.target.value = '';
            return;
        }
...
${validFiles > 0 ? "데이터가 정상입니다. [데이터 복구] 버튼을 눌러 진행하세요." : "⚠️ 유효한 데이터 구조를 찾지 못했습니다."}
                `;
                if (mountedRef.current) alert(report);

            } catch (err: any) {
                console.error(err);
                if (mountedRef.current) alert(`❌ 검사 중 시스템 오류: ${err.message}`);
            } finally {
    ...
    const handleClearApiKey = () => {
        if (!confirm('저장된 API Key를 현재 브라우저 세션에서 제거하시겠습니까?')) return;
