   ...
   return createPortal(
      <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-fade-in" onClick={onClose}>
         <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-5xl overflow-hidden animate-slide-up relative flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <div className="bg-[#0F172A] text-white p-10 relative overflow-hidden shrink-0">
               ...
               <button onClick={onClose} className="absolute top-8 right-8 text-white/50 hover:text-white z-50 p-2 hover:bg-white/10 rounded-full transition-colors"><X size={24} /></button>
               ...
               <div className="relative z-10">
                   <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-6">
                       <div>
                           ...
                           <h2 className="text-4xl font-black leading-tight tracking-tight">안전 성과 <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400">데이터 시각화</span></h2>
                       ...
                       <div className="flex bg-slate-800/50 p-1.5 rounded-xl backdrop-blur-md border border-slate-700/50">
                           {(['WEEK', 'MONTH', 'QUARTER', 'YEAR'] as AnalysisPeriod[]).map((p) => (
                               <button key={p} onClick={() => setPeriod(p)} className={`px-5 py-2 rounded-lg text-xs font-bold transition-all ${period === p ? 'bg-white text-slate-900 shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto">
         ...
         <button onClick={() => onNewEntry('BATCH')} className="relative lg:col-span-2 bg-[#1E293B] rounded-[32px] p-8 text-left shadow-2xl hover:scale-[1.01] transition-all group overflow-hidden border border-slate-700 h-[320px] flex flex-col justify-between">
            ...
            <div className="relative z-10">
                <h3 className="text-4xl font-black text-white mb-3 leading-tight tracking-tight">종합 일지 <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-blue-300">일괄 자동 처리</span></h3>
                <p className="text-sm text-indigo-200/80 font-medium max-w-md leading-relaxed">
         ...
         <button onClick={() => onNewEntry('ROUTINE')} className="relative bg-white rounded-[32px] p-8 text-left shadow-xl hover:scale-[1.01] transition-all group overflow-hidden border border-slate-100 h-[320px] flex flex-col justify-between">
            ...
            <div className="relative z-10">
                <h3 className="text-3xl font-black text-slate-800 mb-3 leading-tight tracking-tight">개별 TBM <br/>간편 등록</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
         ...
         <div onClick={onNavigateToReports} className="rounded-3xl p-6 cursor-pointer relative overflow-hidden group shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 animate-slide-up delay-400 bg-slate-900">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
         {/* Left: Weekly Chart */}
         <div className="lg:col-span-2 bg-white rounded-[32px] p-8 border border-slate-100 shadow-sm flex flex-col animate-slide-up delay-200 relative overflow-hidden">
            <div className="flex justify-between items-center mb-8 relative z-10">
               ...
               <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-100">
                   <button onClick={() => setChartSortBy('COUNT')} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[10px] font-bold transition-all ${chartSortBy === 'COUNT' ? 'bg-white shadow text-slate-900' : 'text-slate-400 hover:text-slate-600'}`}><Activity size={12}/> 참여율순</button>
                   <button onClick={() => setChartSortBy('SCORE')} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[10px] font-bold transition-all ${chartSortBy === 'SCORE' ? 'bg-white shadow text-violet-700' : 'text-slate-400 hover:text-slate-600'}`}><Sparkles size={12}/> AI점수순</button>
               </div>
            </div>

            <div className="relative flex-1 group min-h-[260px] flex items-end">
                {/* Chart Area */}
                <div className="w-full h-full flex items-end gap-4 overflow-x-auto pb-2 custom-scrollbar px-2">
                   {weeklyStats.length === 0 ? (
                      ...
                      weeklyStats.map((stat, idx) => {
                         ...
                         return (
                            <div key={idx} className="flex flex-col items-center group/bar relative h-full justify-end w-14 shrink-0">
                               {/* Tooltip */}
